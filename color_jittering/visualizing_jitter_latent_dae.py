...
from classifier import CIFAR10_DAE_Classifier
from evaluation.loaders import load_jittered_cifar10_for_dae
...
def visualize_latent_space_dae_with_cluster_radius(classifier, dataloader, device, method='tsne', num_samples_per_class=10):
    """
    Visualize the latent space of a DAE-based classifier using t-SNE or PCA and calculate cluster radii for each class.
    
    Args:
        classifier: Trained DAE-powered model.
    ...
    with torch.no_grad():
        for i, (images, lbls) in enumerate(dataloader):
            ...
            latent = classifier.encoder_model(images)
    ...
    plt.grid(True)
    plt.savefig("color_jittering/cifar10_dae_clusters.png")
    plt.show()   
...
if __name__ == "__main__":
    ...
    classifier = CIFAR10_DAE_Classifier(num_classes=10)
    classifier.load_state_dict(torch.load('color_jittering/cifar10_dae_classifier.pth', map_location=torch.device('cpu'), weights_only=True)) 
    classifier.eval()
    print("model loaded")

    visualize_latent_space_dae_with_cluster_radius(classifier, testloader, device='cpu', method='tsne', num_samples_per_class=100)